"""Validation scheduler with workers, throttling, and country-level cooldown."""
...
class ValidationScheduler:
    """Manages concurrent validation with throttling and country-level circuit breaking."""
    ...
    def __init__(self, vat_data: Dict[CountryNumber, VatInfo], callbacks: ValidationCallbacks, stop_event: threading.Event):
        ...
        Args:
            vat_data: Dictionary of VAT data to validate
            callbacks: Callback interface for worker -> UI notifications
    ...
    def validate_batch(self, items: List[Tuple[CountryNumber, VatInfo]]) -> None:
        """Run validation for a batch of VAT items.
        
        Args:
            items: List of (key, VatInfo) tuples to validate
        """
        total = len(items)
        completed = 0
